...
*/
const delay1 = (ms, message) =>
  new Promise((resolve) => {
    setTimeout(() => resolve(message), ms);
  });
async function fetchUserDataAsync(shouldFail) {
  if (shouldFail === false) {
    return await delay1(2000, "User data loaded");
  } else {
    throw await delay1(2000, "Failed to fetch data");
  }
}
...
async function checkoutCart() {
  let stock = await delay(1000, "Checking stock...");
  console.log(stock);

  let Payment = await delay(2000, "Payment processed");
  console.log(Payment);

  let order = await delay(1000, "Order Completed");
  console.log(order);
}
...
async function fetchProductDetails(hasError) {
  try {
    if (hasError === false) {
      return await delay2(3000, "Product details retrieved");
    } else {
      throw await delay2(3000, " Error loading product");
    }
  } catch (error) {
